...
sudo apt-get install xinit arandr cscope slock numlockx offlineimap alsa-utils pulseaudio dmenu aptitude pcmanfm texlive iceweasel flashplugin-nonfree evince mairix eog install-info -y
...
cp .profile .Xauthority .xinitrc .xsessionrc ~
popd
...
# optional packages
sudo apt-get install pv gnome-terminal htop lxappearance network-manager-gnome numix-gtk-theme gnome-system-monitor -y
